...
- **Four scam-guide landing pages, each with its check embedded** — `/puppy-price-check`, `/puppy-shipping-fee-scam`, `/paid-deposit-puppy-scam`, `/teacup-puppy-scam`. SERP research (Aug 2026) found no interactive tool ranking for any of these searches; an AI answer box can summarise an article but can't run a check, so each page pairs cited writing (BBB, FTC, IPATA, the one peer-reviewed pet-scam study) with the tool that answers its question. Prerendered like `/safe`, structurally pinned by `articles.test.js`, content in `frontend/src/content/articles.js`
- **An embeddable scam check for rescues** — `/embed` gives any rescue or shelter a copy-paste iframe of the fee check for their own scam-warning page (free, no signup, no tracking), with a visible credit link in the snippet because an iframe passes no link value and honesty beats hiding it. The widget itself (`/widget/fee-check`) is noindex — its canonical content lives on the shipping-fee page
